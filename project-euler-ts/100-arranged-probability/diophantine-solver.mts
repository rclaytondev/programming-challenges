...
export class DiophantineEquation<VarCount extends number> {
	...
	*solutions(): Generator<Tuple<bigint, VarCount>> {
	...
	modularSolutions(modulo: bigint) {
		...
		const options = new Array(this.varCount).fill([]).map(_ => ArrayUtils.range(0, Number(modulo) - 1).map(BigInt));
		for(const variables of GenUtils.cartesianProduct(...options) as Generator<Tuple<bigint, VarCount>>) {
