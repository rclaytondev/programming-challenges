...
export class Component {
	...
	isReflectionOf(component: Component) {
		return (
			ArrayUtils.equals(this.left, component.right, (r1, r2) => r1.equals(r2)) &&
			ArrayUtils.equals(this.right, component.left, (r1, r2) => r1.equals(r2))
		);
	}
	reflect() {
	...
	static isSymmetric(leftComponents: Component[], rightComponents: Component[]) {
		return ArrayUtils.equals(leftComponents, rightComponents, (c1, c2) => c1.isReflectionOf(c2));
	}
}
export class Range {
	readonly min: number;
	...
	static components(left: number, right: number, blocks: number, components: Component[]): [Component[], Component[]][] {
		...
		for(let numMidRanges = Math.max(1, twoSidedComponents.length); numMidRanges <= blocks; numMidRanges ++) {
			for(const connections of Utils.combinations(components, numMidRanges, "unlimited-duplicates", "tuples")) {
				for(const whichSide of GenUtils.cartesianProduct(...connections.map(c => c.connectableSides()))) {
...
export class SculpturesInitializer {
	...
	static rangeConnections(ranges: Range[]): [Component[], Component[]][] {
		const result = [];
		for(const leftPartition of SetUtils.partitions(ranges)) {
			for(const rightPartition of SetUtils.partitions(ranges)) {
