...
export class GeneratedIterable<T> {
	...
	*[Symbol.iterator]() {
	...
	map<S>(callback: (value: T) => S) {
		// aliasing this is ok because there's no such thing as a generator arrow function
	...
	removeByHash(iterable: GeneratedIterable<T>, hashFunction: (value: T) => string = (x) => `${x}`) {
		// eslint-disable-next-line @typescript-eslint/no-this-alias
		const self = this; // this alias is ok because there's no such thing as generator arrow function
		return new GeneratedIterable(
			function*() {
	...
	static fromIterable<T>(iterable: Iterable<T>, length?: number) {
		if(length == null) {
			length = 0;
			for(const _value of iterable) { length ++; }
		}
		return new GeneratedIterable(
			function*() {
				yield* iterable;
