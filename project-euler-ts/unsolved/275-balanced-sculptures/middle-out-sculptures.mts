...
import { Sculpture } from "./sculpture-subdivision.mjs";
...
export class PartialSculpture {
	...
	nextColumnCombinations(side: "left" | "right", numBlocks: number): Iterable<Set<number>> {
		...
		return GenUtils.subsets(ArrayUtils.range(1, height), numBlocks);
	}
	getChild(leftColumn: number[], rightColumn: number[]) {
		...
		return new PartialSculpture({
			...
			symmetrical: this.symmetrical && SetUtils.equals(leftColumn, rightColumn),
	...
	completions(mode: "symmetrical" | "asymmetrical"): GeneratedIterable<Sculpture> {
		const sideBlocks = new HashSet(this.components.values()).filter((v): v is Vector => Math.abs(v.x) === this.maxX);
	...
	toString() {
		const componentsString = [...this.components.sets().map(set => `[${[...set].sort()}]`)].sort().join(", ");
